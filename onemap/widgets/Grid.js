define(["dojo/dom-attr","dojo/dom-class","dojo/dom",
  ...
  'dojo/aspect',
  'dojo/query',
  'dojo/on',"esri/layers/GraphicsLayer","esri/graphic","esri/geometry/Multipoint","esri/geometry/Polyline","esri/geometry/Polygon","esri/geometry/geometryEngine",
  "esri/toolbars/draw",
  "esri/toolbars/edit","dojo/text!./templates/Grid.html",
...
function(domAttr,domClass,dom,declare,domStyle,lang, array, html,domConstruct,xhr,script, topic, aspect, query,
  on,GraphicsLayer,Graphic,Multipoint,Polyline,Polygon,geometryEngine,Draw,Edit,template,connect, on,_Widget, _Templated,$) {
  var clazz = declare([_Widget, _Templated], {
    ...
    constructor:function(options,srcRefNode){ 
      ...
      this.iWcontentNode = (options.hasOwnProperty("iWcontentNode")) ? options.iWcontentNode :new IWContent({
        "title":title,
        "content":contentStr,
    ...
    addItem:function(item){
      ...
      this.features[idx] = g;
      this.createItem(item.attributes,idx,this.ul);
    },
    postCreate: function() {
      this.inherited(arguments);
...
htmlContent +=`</tbody>               
    ...
    createItem:function(item,idx,srcNodeRef){
      ...
      var li = domConstruct.create("li",{id:"li_oid_"+this.id+"_" + item.OBJECTID},srcNodeRef);
    ...
    containOnclick:function(e,idx){
		...
		{
    		this.starttimestamp=timestamp3;
    		IsContinue=true;
		}
    	else
		{
    		if(timestamp3-this.starttimestamp<500)
    			{
    			IsContinue=false;
    			}

        	console.log(timestamp3-this.starttimestamp);
        	this.starttimestamp=timestamp3;
		}
    	if(IsContinue)
    		{
...
//        var geometry = JSON.parse(geoJson);
    ...
    updateItemUI:function(data,idx,callback){
      /*
       * 编辑完成更新页面
       */
      ...
      var feature = this.features[idx]; 
      feature.setSymbol(this.symbol); 
      lang.mixin(feature.attributes,attr);
      console.log(feature);
      //feature.setAttributes(attr);      
      var g = this.createItem(feature.attributes,idx,null); 
      dojo.place(g,"li_oid_"+this.id+"_" + attr.OBJECTID,"replace");
      //dom.byId("li_oid_" + attr.OBJECTID) = g;
      callback&&callback.apply(this,[feature]);
    },
    removeItem:function(graphic){
      var oid = graphic.attributes.OBJECTID;
      this.listInit();
      var i = dojo.indexOf(this.features,graphic);
      delete this.features[i];
      //this.grid.features.splice(i,1);
      this.dataLayer.remove(graphic);
      this.dataLayer.redraw();
      //TODO
      //刷新页面列表
      domConstruct.destroy(dom.byId("li_oid_"+this.id+"_" + oid));
    },
...
//        };  
    ...
    createList:function(datas,srcNodeRef){
      this.ul =  domConstruct.create("ul",null,srcNodeRef);
      this.dataLayer.clear();
      this.features=[];
      ...
      this.features = dojo.map(datas,lang.hitch(this,function(item,idx){        
        ...
        this.createItem(item.attributes,idx,this.ul);
        if(this.geometries){
          this.geometries.hasOwnProperty("paths")?this.geometries.addPath(item.geometry.paths[0]):
          this.geometries.hasOwnProperty("rings")?this.geometries.addRing(item.geometry.rings[0]):this.geometries.addPoint(item.geometry);
          return g;
        }else{
          this.geometries={};
          switch(g.geometry.type){
          case "point":
            this.geometries = new Multipoint(this.map.spatialReference);
            this.geometries.addPoint(item.geometry);
            return g;
            break;
          case "polyline":
            this.geometries =new Polyline(this.map.spatialReference);
            this.geometries.addPath(item.geometry.paths[0]);
            return g;
            break;
          case "polygon":
            this.geometries =new Polygon(this.map.spatialReference);
            this.geometries.addRing(item.geometry.rings[0])
            return g;
            break;
          default:
      ...
      if(this.features.lenth ==1){
        this.map.centerAndZoom(this.features.geometry,this.mapZoom);
         ...
         var geo = geometryEngine.convexHull(this.geometries);
         console.log(geo);
